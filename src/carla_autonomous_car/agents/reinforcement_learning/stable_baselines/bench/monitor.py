...
class Monitor(gym.Wrapper):
    ...
    def __init__(self,
        ...
        if filename is None:
            ...
            # 修改：使用新的列名 reward, length, time, step
            self.logger = csv.DictWriter(self.file_handler,
                                         fieldnames=('reward', 'length', 'time') + reset_keywords + info_keywords)
    ...
    def step(self, action: np.ndarray) -> Tuple[np.ndarray, float, bool, Dict[Any, Any]]:
        ...
        if done:
            ...
            # 修改：使用新的键名 reward, length, time
            ep_info = {
                "reward": round(ep_rew, 6),
                "length": eplen,
                "time": round((time.time() - self.t_start), 6),

            }
            for key in self.info_keywords:
                ep_info[key] = info.get(key, None)
...
def load_results(path: str) -> pandas.DataFrame:
    ...
    for file_name in monitor_files:
        with open(file_name, 'rt') as file_handler:
            ...
            data_frame['time'] += header['t_start']
        data_frames.append(data_frame)
    data_frame = pandas.concat(data_frames)
    data_frame.sort_values('time', inplace=True)
    data_frame.reset_index(inplace=True)
    data_frame['time'] -= min(header['t_start'] for header in headers)
    return data_frame