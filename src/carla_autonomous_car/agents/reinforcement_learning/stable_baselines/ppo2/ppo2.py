        ...
        with SetVerbosity(self.verbose), TensorboardWriter(self.graph, self.tensorboard_log, tb_log_name, new_tb_log) \
            ...
            for update in range(1, n_updates + 1):
                ...
                if self.verbose >= 1 and (update % log_interval == 0 or update == 1):
                    ...
                    if self.ep_info_buf and len(self.ep_info_buf) > 0:
                        # 过滤有效的 episode 信息
                        valid_ep_infos = [ep for ep in self.ep_info_buf
                                          if isinstance(ep, dict) and 'r' in ep and 'l' in ep]

                        if valid_ep_infos:
                            # 记录平均奖励和长度
                            logger.logkv('ep_reward_mean', safe_mean([ep_info['r'] for ep_info in valid_ep_infos]))
                            logger.logkv('ep_len_mean', safe_mean([ep_info['l'] for ep_info in valid_ep_infos]))
                        else:
                            # 没有有效数据，使用默认值
                            logger.logkv('ep_reward_mean', 0.0)
                            logger.logkv('ep_len_mean', 0.0)
                    else:
                        # buffer 为空，使用默认值
                        logger.logkv('ep_reward_mean', 0.0)
                        logger.logkv('ep_len_mean', 0.0)
