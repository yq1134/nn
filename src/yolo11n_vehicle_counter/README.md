...
├── scripts/
│   ├── yolo_vehicle_counter.py      # 原始版本车辆计数逻辑
...
- **检测置信度**：默认阈值为 0.5，可根据需要调整（CARLA版本支持↑/↓键动态调整）
...
### 交互控制
- 按 'p' 键可以暂停/继续视频播放
- 按 '↑' 键增加置信度阈值，按 '↓' 键降低置信度阈值（仅 CARLA 版本）
- 实时显示处理帧率和计数结果
...
- **扩大ROI区域**：增大YOLO输入尺寸，实现多尺度检测，提高远处车辆的检出率

**🚀 高级功能特性**
- **卡尔曼滤波速度平滑**：使用二维卡尔曼滤波器对车辆速度进行平滑处理，减少数据抖动和噪声
- **实时速度估算**：实时显示每辆车的平滑后速度（m/s），在标签上直观展示
- **轨迹长度统计**：记录并显示每辆车的轨迹长度，便于分析车辆运动模式
- **置信度分布可视化**：实时绘制检测置信度分布条形图，直观展示检测质量
- **置信度阈值动态调整**：支持键盘实时调整检测置信度阈值（↑/↓键），便于调优
- **车辆分类统计**：按汽车、摩托车、公交车、卡车分类统计，实时显示各类别计数

**🎨 可视化增强**
- 半透明覆盖矩形区域
- 圆角边界框标注
- 车辆运动轨迹线
- 实时速度标签（ID + 类型 + 速度 + 轨迹长度）
- 置信度分布面板
...
**🔮 未来优化方向**
- **端到端训练**：将检测、跟踪、计数联合训练，优化整体性能
- **TensorRT部署**：使用TensorRT加速推理，提升处理速度
- **ReID嵌入**：添加重识别特征网络，增强遮挡情况下的跟踪稳定性
- **ByteTrack+ReID组合**：结合外观特征和运动特征，提高长时间跟踪准确率
...
6. **速度平滑**: 卡尔曼滤波器有效减少速度数据的抖动和噪声
7. **多尺度检测**: 增大ROI区域尺寸，提高远处小目标的检出率
